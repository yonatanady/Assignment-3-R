...
```{r Average rodent hindfoot length according to species_id and gender}
...
```{r Create function to calculate meand and sd}
...
ggplot(df2, aes(x = species_id, y = hindfoot_length, fill = sex)) + 
    ...
    theme(axis.text.y = element_text(size = 9))
```
Figure 2.3 | The bar plot shows average hindfoot length by rodent species ID. DS has the longest average hindfoot length, i.e. (`r max(df2$hindfoot_length)`)cm, whereas BA has the shortest, i.e. (`r min(df2$hindfoot_length)`)cm. Most of the species are within the range of 18-22cm. 
